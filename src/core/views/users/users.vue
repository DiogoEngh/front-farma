...
const handleOpenModalRemoveUser = (row) => {
...
const handleChangeUsername = (event) => {
    ...
    payload.username = value?.substring(0, 100)
}

const handleChangeEmail = (event) => {
    const { value } = event.target;
    payload.email = value?.substring(0, 100)
}

const handleChangePassword = (event) => {
    const { value } = event.target;
    payload.password = value?.substring(0, 100)
}
...
<template>
    <div class="flex flex-col gap-4 items-end">
        <div class="w-full">
            <span class="text-xl">Usuários</span>
        </div>
